...
def resolve_discourse_terms_csv(project_root: Path, discourse_party_label: str) -> Path:
    topicos_path = (
        project_root / "data" / "discourses" / "topicos" / discourse_party_label / "lda_topicos_termos.csv"
    )
    if topicos_path.exists():
        return topicos_path

    legacy_path = (
        project_root / "data" / "discourses" / "lda_files" / discourse_party_label / "lda_topicos_termos.csv"
    )
...
def main() -> None:
    ...
    discourse_topics_main(
        ...
        output_base_dir=project_root / "data" / "discourses" / "topicos",
    )
    ...
    discourse_terms_csv = resolve_discourse_terms_csv(project_root, discourse_party_label)
